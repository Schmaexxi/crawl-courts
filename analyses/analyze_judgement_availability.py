...
fig1.savefig("../plots/dejure_vs_old_karlsruhe")
plt.show()


hamburg_dejure_file_numbers = set()
file_prefix = "../data/judgements_dejure_"
file_postfix = "_hamburg.json"
files = ["ag", "arbg", "fg", "lag", "lg", "lsg", "olg", "ovg", "sg", "vg"]
for name in files:
    with open(file_prefix + name +  file_postfix, "r") as f:
        data = json.load(f)

    del data['page']
    for k in data:
        hamburg_dejure_file_numbers.add(k)


with open("../data/judgements_hamburg.json", "r") as f:
    data = json.load(f)

hamburg_file_numbers = set()
for k in data:
    hamburg_file_numbers.add(k)

num_cases_not_in_dejure = len(hamburg_file_numbers - hamburg_dejure_file_numbers)
num_cases_in_dejure = len(hamburg_file_numbers) - num_cases_not_in_dejure

num_cases_not_in_old = len(hamburg_file_numbers - set(open_legal_data_files))
num_cases_in_old = len(hamburg_file_numbers) - num_cases_not_in_old

cases_in = [num_cases_in_dejure, num_cases_in_old]
cases_out = [num_cases_not_in_dejure, num_cases_not_in_old]
fig1, ax1 = plt.subplots()
...
ax1.tick_params(axis='x', which='major', labelsize=9, labelrotation=1)
ax1.set_xlabel("Sources")
ax1.set_title("Availability of judgements of the state of Hamburg \nfrom juris in other sources")
ax1.legend((p1[0], p2[0]), ('Number of cases\navailable', 'Number of cases\nnot available'), loc="lower center")
for i, v in enumerate(cases_in):
    ax1.text(i+0.05, v-v/10, str(v), color='orange', fontweight='bold')
for i, v in enumerate(cases_out):
    ax1.text(i+0.085, cases_in[i] + v/10, str(v), color='blue', fontweight='bold')
fig1.savefig("../plots/dejure_vs_ol_hamburg")
plt.show()